@testset "Contact Sampling" begin
    ...
    @testset "AgeBasedContactSampling" begin
        ...
        @test length(sample_contacts(abcs_null, hnull, i_index, individuals(hnull), GEMS.DEFAULT_TICK)) == 0

        # AgeBasedContactSampling should return vectors of individuals 
        @test typeof(sample_contacts(abcs_null, hnull, i_index, individuals(hnull), GEMS.DEFAULT_TICK)) == Vector{Individual}
        ...
        @test mean([length(sample_contacts(abcs1, h1, i_index, individuals(h1), GEMS.DEFAULT_TICK)) for _ = 1:1000]) < 2
        @test mean([length(sample_contacts(abcs1, h1, i_index, individuals(h1), GEMS.DEFAULT_TICK)) for _ = 1:1000]) > 0
        # expected count equal 2
        @test mean([length(sample_contacts(abcs2, h2, i_index, individuals(h2), GEMS.DEFAULT_TICK)) for _ = 1:1000]) < 3
        @test mean([length(sample_contacts(abcs2, h2, i_index, individuals(h2), GEMS.DEFAULT_TICK)) for _ = 1:1000]) > 1
        # expected count equal 3
        @test mean([length(sample_contacts(abcs3, h3, i_index, individuals(h3), GEMS.DEFAULT_TICK)) for _ = 1:1000]) < 4
        @test mean([length(sample_contacts(abcs3, h3, i_index, individuals(h3), GEMS.DEFAULT_TICK)) for _ = 1:1000]) > 2
        # expected count equal 100
        @test mean([length(sample_contacts(abcs100, h100, i_index, individuals(h100), GEMS.DEFAULT_TICK)) for _ = 1:1000]) < 101
        @test mean([length(sample_contacts(abcs100, h100, i_index, individuals(h100), GEMS.DEFAULT_TICK)) for _ = 1:1000]) > 99

        # create empty setting
        empty_h = Household(id = 2, individuals = Vector{Individual}(), contact_sampling_method = abcs1)

        # Sampling from a setting where no individual is present should result in an error
        @test_throws ArgumentError sample_contacts(abcs1, empty_h, i_index, individuals(empty_h), GEMS.DEFAULT_TICK)
