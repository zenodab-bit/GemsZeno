...
function calculate_progression(individual::Individual, tick::Int16, dp::Critical, rng::Xoshiro)
    ...
    hospital_admission = Int16(severeness_onset + rand_val(dp.severeness_onset_to_hospital_admission, rng))

    # Calculate the time to ICU admission
    icu_admission = Int16(hospital_admission + rand_val(dp.hospital_admission_to_icu_admission, rng))

    # Decide recovery or death
    # If individual dies
    if gems_rand(rng) <= dp.death_probability
        # Calculate the time to death
        death = Int16(icu_admission + rand_val(dp.icu_admission_to_death, rng))
    ...
    icu_discharge = Int16(icu_admission + rand_val(dp.icu_admission_to_icu_discharge, rng))

    # Calculate the time to hospital discharge
    hospital_discharge = Int16(icu_discharge + rand_val(dp.icu_discharge_to_hospital_discharge, rng))

    # Calculate the time to severeness offset
    severeness_offset = Int16(hospital_discharge + rand_val(dp.hospital_discharge_to_severeness_offset, rng))
