...
* simulate different non-pharamaceutical interventions (NPIs), 
* account for the different contact structures in different settings an individual can be in (e.g. households or offices)
* handle various vaccination strategies and multiple vaccines
...
#### Installation of Required Software as Prerequisite

We recommend using VSCode to work with GEMS.   
In case you are new to Julia or VSCode, you might need to install and setup additional software for GEMS to run.
...
```@raw html
<details><summary style="font-size: 1.3em;">Installation Guide</summary>
```

```@raw html
<details><summary style="font-size: 1.1em;">Windows</summary>
```
...
#### 1. Installing Julia
The GEMS framework was developed under Julia v.1.10.4. 
 
1. Download the Windows installer for version v.1.10.4 of [Julia](https://julialang.org/downloads/) from the JuliaLang website.
...
#### 2. Installing MiKTex and Pandoc
...
```@raw html
<details><summary style="font-size: 1.1em;">Mac OS</summary>
```
...
2. Download the MacOS installer (usually a .dmg file) for your MacOS system for version v.1.10.4, i.e., the newest version, of Julia.
