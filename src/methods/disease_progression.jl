...
"""
    sample_mild_death_rate(pathogen::Pathogen, indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_severe_death_rate(pathogen::Pathogen, indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_critical_death_rate(pathogen::Pathogen, indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_hospitalization_rate(pathogen::Pathogen, indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_ventilation_rate(pathogen::Pathogen, indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_icu_rate(pathogen::Pathogen, indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_self_quarantine_rate(pathogen::Pathogen, indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
###
"""
    sample_time_to_recovery(pathogen::Pathogen,  indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_onset_of_symptoms(pathogen::Pathogen,  indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_infectious_offset(pathogen::Pathogen,  indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_onset_of_severeness(pathogen::Pathogen,  indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_time_to_hospitalization(pathogen::Pathogen,  indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_length_of_stay(pathogen::Pathogen,  indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    sample_time_to_icu(pathogen::Pathogen,  indiv::Individual; rng::AbstractRNG = Random.default_rng())
...
"""
    estimate_disease_progression(dpr::DiseaseProgressionStrat, indiv::Individual; rng::AbstractRNG = Random.default_rng())

Estimates the final status an individual reaches during the progression of a disease.
"""
function estimate_disease_progression(dpr::DiseaseProgressionStrat, indiv::Individual; rng::AbstractRNG = Random.default_rng())::DataType
...
"""
    disease_progression!(infectee::Individual, pathogen::Pathogen, exposedtick::Int16; rng::AbstractRNG = Random.default_rng())

Assigns a disease progression to the `infectee` by assigning event ticks based on the 
`exposedtick`.
"""
function disease_progression!(infectee::Individual, pathogen::Pathogen, exposedtick::Int16; rng::AbstractRNG = Random.default_rng())
    ...
    disease_progression!(infectee, pathogen, exposedtick, estimated_symptom_category, rng=rng)
end
...
function disease_progression!(
    ...
    theoretical_onset_of_symptoms = sample_onset_of_symptoms(pathogen, infectee, rng=rng)
    infectious_tick!(
        infectee,
        # prevent setting infectiouness to BEFORE infection
        max(exposedtick + Int16(1), exposedtick + 
            theoretical_onset_of_symptoms -
            sample_infectious_offset(pathogen, infectee, rng=rng))
    )
    ...
    removed_tick!(
        ...
        exposedtick + 
            theoretical_onset_of_symptoms +
            sample_time_to_recovery(pathogen, infectee, rng=rng)
    )
...
function disease_progression!(
    ...
    onset_of_symptoms!(
        infectee,
        max(exposedtick + Int16(1), exposedtick + sample_onset_of_symptoms(pathogen, infectee, rng=rng))
    )
    # decrement by random tick value drawn for the time to infectiousness
    infectious_tick!(
        infectee,
        # prevent setting infectiouness to BEFORE infection
        max(exposedtick + Int16(1), onset_of_symptoms(infectee) - sample_infectious_offset(pathogen, infectee, rng=rng))
    )
    # removed tick as normal
    removed_tick!(
        infectee,
        onset_of_symptoms(infectee) + sample_time_to_recovery(pathogen, infectee, rng=rng)
    )
...
function disease_progression!(
    ...
    onset_of_severeness!(
        infectee,
        onset_of_symptoms(infectee) + sample_onset_of_severeness(pathogen, infectee, rng=rng)
    )
    ...
    hospitalization_probability = sample_hospitalization_rate(pathogen, infectee, rng=rng)
    ...
    if rand(rng) < hospitalization_probability
        ...
        hospitalized_tick!(
            ...
            onset_of_symptoms(infectee) +
                sample_time_to_hospitalization(pathogen, infectee, rng=rng)
        )
