...
"""
    process!(batch::Batch; seed=nothing, representative_by=pp->nrow(infectionsDF(pp)), keep_rundata=false, rd_style="LightRD")
...
- `representative_by`: a function `pp::PostProcessor -> scalar` for selecting a
  representative run. The simulation with criterion closest to the median across
  all runs is re-run and stored. For multi-label batches one representative per
  label is computed. Default: total infections. Pass `nothing` to disable.
- `keep_rundata`: if `true`, every run's `ResultData` is stored in `bp.rundata`.
  Required for `merge(bds::BatchData...)`. Default: `false`.
- `rd_style`: the `ResultData` style for individual and representative runs.
  Default: `"LightRD"`.
"""
function process!(batch::Batch;
    seed::Union{Nothing, Integer} = nothing,
    representative_by::Union{Nothing, Function} = pp -> nrow(infectionsDF(pp)),
    ...
    collect_rep = representative_by !== nothing
    criterion_values = !multi_label && collect_rep ? Float64[] : nothing
    label_criteria = multi_label && collect_rep ? Dict{String, Vector{Float64}}() : nothing
    label_indices = multi_label && collect_rep ? Dict{String, Vector{Int}}() : nothing
    ...
    for (i, (cfg, sim_label)) in enumerate(zip(configs, cfg_labels))
        ...
        if collect_rep
            val = Float64(representative_by(pp))
    ...
    if multi_label
        _run_label_representatives!(bp, configs, sim_seeds, label_criteria, label_indices, rd_style)
    else
        _run_representative!(bp, configs, sim_seeds, criterion_values, rd_style)
    end

    return bp
end

function _run_representative!(bp, configs, sim_seeds, criterion_values, rd_style)
    ...
    printinfo("Re-running representative simulation (config $best_idx)")
    rep_sim = Simulation(; configs[best_idx]..., seed = sim_seeds[best_idx])
    run!(rep_sim)
    bp.representative_run = ResultData(PostProcessor(rep_sim), style = rd_style)
end

function _run_label_representatives!(bp, configs, sim_seeds, label_criteria, label_indices, rd_style)
    ...
    for (lab, criteria) in label_criteria
        ...
        printinfo("Re-running representative simulation for label \"$lab\" (config $best_idx)")
        rep_sim = Simulation(; configs[best_idx]..., seed = sim_seeds[best_idx])
        run!(rep_sim)
        bp.per_label[lab].representative_run = ResultData(PostProcessor(rep_sim), style = rd_style)
    end
end
