...
# Include custom population module
...
# Initialize and run the base simulation without concert event
sim_base = Simulation(
...
gp = gemsplot(rd_base)
png(gp, "Concert Project/Plots/Single_general_base_2.png")
...
rd_base.data["dataframes"]["tick_cases_per_setting"] =
    filter(row -> row.setting_type in ['h', 's', 'w', 'g'], tick_cases_base)

# Generate a plot of cases by setting
gp1 = gemsplot(rd_base, type = :TickCasesBySetting)
png(gp1, "Concert Project/Plots/Single_cases_base_2.png")
...
# Sum the daily cases for GlobalSetting
